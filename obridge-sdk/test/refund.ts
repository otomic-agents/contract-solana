...
import { createAccountOnChain, createSPLTokenAndMintToUser, sleep } from './utils';
...
async function main() {
    ...
    const amount = 2 * 10 ** 9;
    let { mint: mint1, ataTokenAccount: userAtaTokenMint1Account } = await createSPLTokenAndMintToUser(
        connection,
        payer,
        user,
        amount,
    ...
    let { mint: mint2, ataTokenAccount: lpAtaTokenMint2Account } = await createSPLTokenAndMintToUser(
        connection,
        payer,
        lp,
        amountBack,
    ...
    tx = await obSrv.refundTransferOut(uuid1, userAtaTokenMint1Account.address, escrow1, escrow1Ata, TOKEN_PROGRAM_ID);
    txHash = await obSrv.sendTransaction(tx, [payer]);
    console.log(`refund transfer out tx: ${txHash}`);

    // refund transfer in
    console.log(`========== refund transfer in ==========`);
    tx = await obSrv.refundTransferIn(uuid2, lpAtaTokenMint2Account.address, escrow2, escrow2Ata, TOKEN_PROGRAM_ID);
    txHash = await obSrv.sendTransaction(tx, [payer]);
