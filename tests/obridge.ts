...
describe("obridge", async () => {
    ...
    it("SPL A_Token -> SPL B_Token", async () => {
        ...
        console.log(`init balance: ${userBal / web3.LAMPORTS_PER_SOL} SOL`);
        ...
        const lp = await createAccountOnChain(user, connection);
        ...
        let escrowAtaTokenAccount = getAssociatedTokenAddressSync(mint1, escrow, true)
        console.log(`calculated offchain ata address ${escrowAtaTokenAccount} of user ${escrow} SLP token mint1 ${mint1}`)
        ...
        const tx = await program.methods
            ...
            .accounts({
                ...
                escrow: escrow,
                escrowAta: escrowAtaTokenAccount,
...
async function createAccountOnChain(payer: web3.Keypair, connection: web3.Connection): Promise<web3.Keypair> {
